...
(() => {
  ...
  function buildCsv(results, participantId, aborted, totalPlanned) {
    const header = [
      "trial","session","block","group","condition","talker","word","word_count",
      "image_file","audio_file","image_onset_time","audio_onset_time","response_time",
      "session_status","planned_trials","participant_id"
    ];
    ...
    if (results.length === 0) {
      rows.push([0,0,0,0,0,0,'NA',0,'NA','NA','NA','NA','NA', aborted ? 'aborted' : 'completed', totalPlanned, participantId].join(','));
    }
    return rows.join('\n');
  }
  ...
  startBtn.addEventListener('click', async () => {
    ...
    try {
      ...
      const csv = buildCsv(results, participantId, aborted, schedule.trials.length);
      ...
      setStatus(aborted ? '中断しました。結果（途中まで）をダウンロードできます。' : '結果をダウンロードしてください');
      setLog(`トライアル数: ${results.length} / 計画: ${schedule.trials.length} / ステータス: ${aborted ? 'aborted' : 'completed'}`);
      preloadBtn.disabled = false;
    } catch (err) {
      console.error(err);
      setStatus(`エラー: ${err.message}`);
